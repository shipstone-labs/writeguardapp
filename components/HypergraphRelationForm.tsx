...
export function HypergraphRelationForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const { valid, errors: validationErrors } = validateRelationData(
      "Similarity",
    ...
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
        ...
        <div>
          ...
          {availableEntities.length > 0 ? (
            <select
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, fromId: e.target.value }))
              }
          ...
          <p className="text-sm text-gray-500 mt-1">
            The entity this relation starts from
          </p>
        </div>
        ...
        <div>
          ...
          {availableEntities.length > 0 ? (
            <select
              ...
              value={formData.toId}
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, toId: e.target.value }))
              }
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              required
            >
        ...
        <div>
          ...
          <div className="mt-2">
            <input
              ...
              value={formData.closeness}
              onChange={(e) =>
                handleClosenessChange(parseFloat(e.target.value) || 0)
              }
              min="0"
              max="1"
              step="0.01"
          ...
          {formData.closeness > 0.8 && (
            <div className="mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
              <p className="text-sm text-yellow-800">
                ⚠️ High similarity detected (&gt;0.8). This may indicate
                potential plagiarism or duplicate content.
              </p>
            </div>
          )}
        ...
        {formData.fromId && formData.toId && (
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-md">
            ...
            <p className="text-sm text-blue-700">
              ...
              {formData.violation && (
                <span className="text-red-600"> (⚠️ VIOLATION)</span>
              )}
            </p>
          </div>
        )}
      ...
      <div className="mt-8 p-4 bg-gray-50 rounded-md">
        ...
        <div className="text-sm text-gray-600 space-y-1">
          <p>
            <strong>Relation Type:</strong> {SIMILARITY_RELATION_TYPE.name}
          </p>
          <p>
            <strong>Description:</strong> {SIMILARITY_RELATION_TYPE.description}
          </p>
          <p>
            <strong>Violation Threshold:</strong> Closeness &gt; 0.8
          </p>
          <p>
            <strong>GraphQL Query:</strong> Find violations with{" "}
            <code>relations(where: {"{violation: true}"})</code>
          </p>
        </div>
      </div>
    </div>
